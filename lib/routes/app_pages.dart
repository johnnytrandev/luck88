      ...
      page: () => Sizer(
        builder: (context, orientation, deviceType) {
          return FutureBuilder(
            future: Future(() async {
              final response = await http.get(Uri.parse('https://nvtrong1906.github.io/luck.json'));

            print('luckData response ${jsonDecode(response.body)}');
            ...
            print('luckData snapshot ${snapshot.hasData}');
            print('luckData snapshot data ${snapshot.data}');
              // if (snapshot.hasData) {
            print('luckData snapshot.hasData ${snapshot.hasData}');
                // final constraints = futureContext.layoutConstraints;
                // Use constraints to determine layout or widget sizes
                return AnnotatedRegion<SystemUiOverlayStyle>(
                  value: const SystemUiOverlayStyle(
                    statusBarColor: AppColor.transparent,
                    statusBarIconBrightness: Brightness.dark,
                  ...
                  child: HomeScreen(luckData: snapshot.data),
                );
              // } else {
              //   return AnnotatedRegion<SystemUiOverlayStyle>(
              //     value: const SystemUiOverlayStyle(
