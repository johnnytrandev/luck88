  ...
  _argumentData() {
    if (arguments != null) {
      ...
      if (arguments[4] != null) {
        ...
        Debug.printLog(
            "listOfAnimationController -->>$workoutPlanData");
      }
    }
  }
  ...
  void onInit() {
    ...
    if (exerciseList[currentPos + 1].exUnit == Constant.workoutTypeStep) {
      Utils.textToSpeech("tts6".tr, MyApp.flutterTts).then((value) {
        Utils.textToSpeech(
            "${"tts7".tr} ${exerciseList[currentPos + 1].exTime} ${"tts8".tr} ${Utils.getMultiLanguageString(exerciseList[currentPos + 1].exName!)}",
            MyApp.flutterTts);
      });
    } else {
      Utils.textToSpeech("tts6".tr, MyApp.flutterTts).then((value) {
        Utils.textToSpeech(
            "${"tts7".tr} ${exerciseList[currentPos + 1].exTime} ${"tts4".tr} ${Utils.getMultiLanguageString(exerciseList[currentPos + 1].exName!)}",
            MyApp.flutterTts);
      });
    }
    super.onInit();
