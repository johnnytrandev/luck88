...
class CompletedController extends GetxController {
  ...
  onShareButtonClick() {
    ...
    var strText = "${"txtIHaveFinish".tr} ${exerciseList.length} ${"txtOf".tr} ${"txtAppName".tr} ${"txtExerciseDot".tr}\n${"txtYouShould".tr} $link";
  ...
  onWeightFieldSubmitted(String value) async {
    ...
    if (value.isEmpty) {
      ...
      Utils.showToast(Get.context!, "txtWarningForKg".tr);
        ...
        (double.parse(value) < Constant.minLB ||
            double.parse(value) > Constant.maxLB)) {
      Utils.showToast(Get.context!, "txtWarningForLb".tr);
    } else {
      ...
      if (weightUnit != Constant.weightUnitKg) {
        ...
        Preference.shared
            .setInt(Preference.currentWeightInLbs, double.parse(value).round());
      } else {
        Preference.shared
            .setInt(Preference.currentWeightInKg, double.parse(value).round());
        Preference.shared.setInt(Preference.currentWeightInLbs,
            Utils.convertWeightKgToLbs(double.parse(value)));
      }
    }
  }
  ...
  setWeightValues() {
    ...
    } else if (weightUnit == Constant.weightUnitLbs && lastWeightKg != 0) {
